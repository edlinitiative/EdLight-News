...
export async function listRecentSent(sinceHours = 24, limit = 50): Promise<XQueueItem[]> {
  ...
  const snap = await collection()
    ...
    .get();
  return snap.docs
    .map((d) => ({ id: d.id, ...d.data() }) as XQueueItem)
    .filter((item) => {
...
export async function listSentToday(limit = 20): Promise<XQueueItem[]> {
  const { startTs } = haitiDayBounds();
  const snap = await collection()
    .where("status", "==", "sent" satisfies XQueueStatus)
    .where("updatedAt", ">=", startTs)
...
/**
 * Patch social engagement metrics onto a sent X queue item.
 * Called by pullSocialMetrics (P2) after fetching X tweet public_metrics.
 */
