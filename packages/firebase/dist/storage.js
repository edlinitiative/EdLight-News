 ...
 */
export async function uploadImageBuffer(path, buffer, contentType = "image/png") {
 ...
 */
export async function uploadCarouselSlides(localPaths, queueItemId) {
    ...
    console.log(`[storage] Uploaded ${urls.length} carousel slides for IG queue item ${queueItemId}`);
    return urls;
}
/**
 ...
 */
export async function uploadStorySlide(localPath, storyId, frameIndex) {
    const { readFileSync } = await import("node:fs");
    const buffer = readFileSync(localPath);
