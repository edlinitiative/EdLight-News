...
import { createUtilitySourceSchema } from "@edlight-news/types";
const COLLECTION = "utility_sources";
function collection() {
    return getDb().collection(COLLECTION);
}
export async function createUtilitySource(data) {
...
export async function getUtilitySource(id) {
...
export async function listBySeries(series) {
...
export async function listBySeriesAndRotation(series, rotationKey) {
    const all = await listBySeries(series);
    return all.filter((s) => !s.rotationKey || s.rotationKey === rotationKey);
}
/** Upsert a utility source by URL. Used for seeding. */
export async function upsertByUrl(data) {
    const validated = createUtilitySourceSchema.parse(data);
    ...
    if (!existing.empty) {
        ...
        return { source: { id: doc.id, ...snap.data() }, created: false };
    }
    const source = await createUtilitySource(data);
    return { source, created: true };
}
//# sourceMappingURL=utility-sources.js.map