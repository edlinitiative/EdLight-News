...
describe("buildStorySlideHTML", () => {
  it("CTA frame uses @edlight.news handle", () => {
    ...
    assert.ok(!html.includes("@edlightnews"), "Should NOT contain @edlightnews (without dot)");
  });
  ...
  it("dispatches taux frameType to financial card", () => {
    ...
    assert.ok(html.includes("TAUX DU JOUR"), "Should contain TAUX DU JOUR pill");
  ...
  it("dispatches headline frameType to article summary card", () => {
    const slide: IGStorySlide = {
      heading: "BOURSE — Bourse UNESCO 2026",
      bullets: ["Résumé de l'article", "Source: Le Nouvelliste"],
      accent: "#3b82f6",
      frameType: "headline",
    };
    const html = buildStorySlideHTML(slide, "13 mars 2026", 2, 5);
    assert.ok(html.includes("Bourse UNESCO 2026"), "Should contain headline");
    // Source line should be separated from main bullets
  ...
  it("legacy slides without frameType use positional fallback", () => {
    ...
    assert.ok(coverHtml.includes("Résumé du jour"), "Cover should render heading");
    ...
    const headlineHtml = buildStorySlideHTML(headlineSlide, "13 mars 2026", 1, 3);
  ...
  it("CTA via frameType (not isCta flag)", () => {
    ...
    assert.ok(html.includes("@edlight.news"), "CTA via frameType should have handle");
  });

  it("all frames have 1080×1920 dimensions", () => {
    const tauxSlide: IGStorySlide = { heading: "131.00", bullets: ["date"], frameType: "taux" };
    const factsSlide: IGStorySlide = { heading: "Facts", bullets: ["f1"], frameType: "facts" };
    const headlineSlide: IGStorySlide = { heading: "Headline", bullets: ["b1"], frameType: "headline" };
    ...
    for (const [label, slide, idx] of [
      ...
      assert.ok(html.includes("1080px"), `${label} frame should be 1080px wide`);
      assert.ok(html.includes("1920px"), `${label} frame should be 1920px tall`);
    }
  });
});
