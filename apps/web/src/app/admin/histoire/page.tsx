    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        <div className="rounded-xl border p-5 bg-white">
          <p className="text-sm text-stone-500">Almanac entries</p>
          <p className="mt-1 text-3xl font-bold">{entries.length}</p>
          <p className="text-xs text-stone-400">{highConfidence} high confidence</p>
        </div>
        <div className="rounded-xl border p-5 bg-white">
          <p className="text-sm text-stone-500">Holidays</p>
          <p className="mt-1 text-3xl font-bold">{holidays.length}</p>
          <p className="text-xs text-stone-400">{holidays.filter((h) => h.isNationalHoliday).length} national</p>
        </div>
        <div className="rounded-xl border p-5 bg-white">
          <p className="text-sm text-stone-500">Month coverage</p>
          <p className="mt-1 text-3xl font-bold">{monthCoverage}/12</p>
          <p className="text-xs text-stone-400">{uniqueDays} unique days</p>
        </div>
        <div className="rounded-xl border p-5 bg-white">
          <p className="text-sm text-stone-500">Recent publishes</p>
      ...
      <section className="space-y-3">
        ...
        {recentLogs.length > 0 ? (
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-stone-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-medium text-stone-500">Entries</th>
                  <th className="px-4 py-2 text-left font-medium text-stone-500">Item ID</th>
                  <th className="px-4 py-2 text-left font-medium text-stone-500">Error</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {recentLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-stone-50">
                    ...
                    <td className="px-4 py-2 text-xs text-stone-600">{log.almanacEntryIds.length}</td>
                    <td className="px-4 py-2 text-xs text-stone-400 font-mono">
          ...
          <p className="text-sm text-stone-400">No publish logs yet.</p>
        )}
      </section>
        ...
        <div className="overflow-hidden rounded-lg border">
          <table className="min-w-full text-sm">
            <thead className="bg-stone-50">
              <tr>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Date</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Name (FR)</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Name (HT)</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">National</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {holidays
                ...
                .map((h) => (
                  <tr key={h.id} className="hover:bg-stone-50">
                    ...
                    <td className="px-4 py-2 text-stone-600">{h.name_ht}</td>
        ...
        <div className="overflow-hidden rounded-lg border">
          <table className="min-w-full text-sm">
            <thead className="bg-stone-50">
              <tr>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Date</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Year</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Title</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Tags</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Confidence</th>
                <th className="px-4 py-2 text-left font-medium text-stone-500">Sources</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {entries
                ...
                .map((e) => (
                  <tr key={e.id} className="hover:bg-stone-50">
                    ...
                    <td className="px-4 py-2 text-xs text-stone-500">{e.year ?? "—"}</td>
                    ...
                    <td className="px-4 py-2">
                      <div className="flex flex-wrap gap-1">
                        {e.tags?.slice(0, 3).map((tag) => (
                          <span
                            key={tag}
                            className="rounded-full bg-stone-100 px-1.5 py-0.5 text-[9px] text-stone-600"
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    <td className="px-4 py-2 text-xs text-stone-400">{e.sources.length}</td>
                  </tr>
                ))}
            </tbody>
          </table>
