...
import { thQueueRepo, getDb } from "@edlight-news/firebase";
...
function docToItem(doc: FirebaseFirestore.QueryDocumentSnapshot) {
  ...
  return {
    ...
    thPostId: (data.thPostId as string | null) ?? null,
...
export async function GET() {
  try {
    ...
    const activePromise = db
      ...
      .get()
      .catch((err) => {
        console.warn("[api/admin/th-queue] activeSnap query failed (missing index?):", err);
        return null;
      });

    const countPromise = db.collection("th_queue").count().get().catch(() => null);
    ...
    console.error("[api/admin/th-queue] GET error:", err);
    return NextResponse.json(
      { error: err instanceof Error ? err.message : "Failed to load Threads queue" },
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (action === "publish_now") {
      ...
      await thQueueRepo.setScheduled(id, now);
      return NextResponse.json({
        ok: true,
        action,
        scheduledFor: now,
        message: "Scheduled for immediate Threads publishing on the next tick.",
      });
    }

    if (action === "skip") {
      await thQueueRepo.updateStatus(id, "skipped", {
    ...
    if (action === "requeue") {
      await thQueueRepo.updateStatus(id, "queued", {
        sendRetries: 0,
        error: null,
      });
      return NextResponse.json({ ok: true, action });
    ...
    console.error("[api/admin/th-queue] PATCH error:", err);
    return NextResponse.json(
