      ...
      <div className="space-y-6">
        {pathways.map((p) => {
          ...
          return (
            <div
              key={p.id}
              className="overflow-hidden rounded-lg border bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800"
            >
              {/* Pathway header */}
              <div className="bg-gradient-to-r from-brand-50 to-indigo-50 dark:from-brand-900/20 dark:to-indigo-900/20 p-6">
                ...
                <div className="divide-y dark:divide-slate-700">
                  {p.steps.map((step, idx) => (
                      <div key={idx} className="flex gap-4 p-4">
                        {/* Step number */}
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-100 text-sm font-bold text-brand-700 dark:bg-brand-900/30 dark:text-brand-300">
                          {idx + 1}
                        </div>
              ...
              {p.sources && p.sources.length > 0 && (
                <div className="border-t bg-gray-50 p-4 dark:border-slate-700 dark:bg-slate-800">
